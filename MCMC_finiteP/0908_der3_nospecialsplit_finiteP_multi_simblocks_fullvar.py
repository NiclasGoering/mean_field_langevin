# multimode_quenched_fixedD_multi_runs_diag.py
...
def saem_multimode_quenched_fixedD_multi_runs(
    ...
    for it in range(1, saem.max_iters + 1):
        ...
        # M-step (diagonal): per-block cross-fitted scalar updates per mode
        for slot in per_dev:
            device = slot["device"]
            ...
            # Shapes: J1/J2: (K,B,M) ; invD1/invD2: (K,B)
            B1_diag = torch.einsum('kbm,kb->km', J1 * J1, invD1) / float(J1.shape[1])  # (K,M)
            B2_diag = torch.einsum('kbm,kb->km', J2 * J2, invD2) / float(J2.shape[1])  # (K,M)
            B_diag  = 0.5 * (B1_diag + B2_diag)                                        # (K,M)

            # Cross-fitted b: (1) w12 uses J_S from half-2 with invD1, (2) w21 uses J_S from half-1 with invD2
            J1_S = J1[:, :, 0]                                 # (K,B) signal mode from half-1
            J2_S = J2[:, :, 0]                                 # (K,B) signal mode from half-2
            w12  = J2_S * invD1                                # (K,B)
            w21  = J1_S * invD2                                # (K,B)
            ...
            # Scalar (per-mode) update: m* = N b / (1 + N B + ridge)
            denom = 1.0 + mdl.N * B_diag + saem.ridge_lambda
    ...
    summary = {
        "kappa": kappa, "P": P, "mode": "multi-quenched-fixedD-parallel-xfit-diag",
...
if __name__ == "__main__":
    ...
    BASE = dict(
        ...
        ridge_lambda=1e-4,
        blocks=2,                       # parallel datasets (replicates)
    ...
    save_dir = "/home/goring/mean_field_langevin/MCMC_finiteP/results/2208_diag_Mstep_grid"
    os.makedirs(save_dir, exist_ok=True)
    run_tag_prefix = time.strftime("%Y%m%d_%H%M%S")

    # >>> Set your sweeps here <<<
    kappa_list = [1e-3]
    P_list     = [100] #[10, 100, 250, 500, 600, 700, 800, 900, 1000, 2000]
    M_noise    = 2048

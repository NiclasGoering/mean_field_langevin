# schur_fullkernel_finiteP.py
# Fully runnable. Adds self-consistent sparsity gates to the finite-P mean-field solver.
# - Each sampled neuron carries an activation probability p_active = σ(-ΔF)
# - Kernel/b-vector are gate-weighted -> only "active" samples contribute strongly
# - Optionally replace N with N_eff = ρN inside α(w) via rho_for_Acoef (set each outer step)
...
class MCMCParams:
    B: int = 8192          # chains
    steps: int = 50        # SGLD steps per outer iter
...
# ----------------------------- SGLD sampler (with sparsity gates) -----------------------------
@dynamo.disable
def sgld_sample_w(
    w: torch.Tensor,      # (B,d)
    ...
    mcmc: MCMCParams,
    *,
    rho_for_Acoef: Optional[float] = None,  # if given, α uses N_eff = ρN
    prior_logit: float = 0.0                # logit prior for gate, default 0 (uninformative)
    ...
    Returns:
      ...
      - p_active  (B,) = σ(prior_logit - ΔF(w))  (sparsity gate)
    """
    device = w.device
    P = X.shape[0]
    ...
    # Allow effective Acoef using ρN if provided; else use full N
    N_eff = mdl.N if (rho_for_Acoef is None) else max(1e-9, mdl.N * float(rho_for_Acoef))
    Acoef = (N_eff ** mdl.gamma) / mdl.sigma_a      # α base term
    ...
    for _ in range(mcmc.steps):
        ...
        # ----- Final recompute for outputs & gates -----
    with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=autocast_enabled):
        z = X @ w.t().contiguous()
    ...
    # α and D_oracle using same Acoef as above
    D_internal = (Acoef + Sigma / (kappa**2)).clamp_min(1e-9)   # α(w)
    D_oracle   = (Acoef * (kappa**2) + Sigma).clamp_min(1e-9)   # κ² α(w)

    # Couplings
    JY_final = (g.t() @ y_f) / float(P)                          # (B,)
    Jm_final = (g.t() @ f_mean) / float(P)                       # (B,)

    # --- NEW: kappa-stable gate energy (O(1) scale)
    # ΔF_gate = 0.5*log(D_oracle) - 0.5*(JY-Jm)^2 / D_oracle
    DeltaF_gate = 0.5*torch.log(D_oracle) - 0.5*(JY_final - Jm_final).pow(2) / (D_oracle + 1e-12)

    # Optional gate temperature (leave at 1.0 to avoid a new free parameter)
    tau_gate = 1
    p_active = torch.sigmoid((prior_logit - DeltaF_gate) / tau_gate).detach()

    return w.detach(), phi.detach(), D_oracle.detach(), D_internal.detach(), p_active
...
class FullKernelOracle:
    ...
    Implements (I + N B) m = N b using an implicit full kernel:
      ...
      With gates: each sample b contributes with weight gate_b ∈ [0,1].

      Khat v  := (1/B) * sum_b  [ gate_b * φ^(b) * ((φ^(b)^T v) / D_b) ]   ∈ R^P
    ...
    def __init__(self, C_pm: torch.Tensor, y: torch.Tensor, N: int):
        ...
        self.gate = None         # (1,B) float in [0,1]

        # Precomputed for diag preconditioner
        self.diag_B_scalar = None

    def update_samples(self, phi: torch.Tensor, D_oracle: torch.Tensor, gate: Optional[torch.Tensor] = None):
        """
        Register new φ, D and optional gate weights.
        gate: (B,) or (1,B) tensor with values in [0,1]. If None, uses all-ones (no gating).
        """
        ...
        if gate is None:
            self.gate = torch.ones(1, B, device=self.device, dtype=torch.float32)
        else:
            g = gate.detach().to(self.device).float()
            if g.dim() == 1:
                g = g.view(1, -1)
            self.gate = g  # (1,B)

        # For diag preconditioner: sum_μ w_μ with w_μ = mean_b[ gate_b * φ_μb^2 / D_b ]
        invD = (self.gate / self.D_oracle.view(1, B))     # (1,B)
        w_mu = (self.phi*self.phi) * invD                 # (P,B)
    ...
    def _Khat(self, vP: torch.Tensor) -> torch.Tensor:
        """
        vP ∈ R^P -> (1/B) Σ_b [ gate_b * φ_b * ((φ_b^T vP)/D_b) ] ∈ R^P
        """
        ...
        weights = (self.gate.view(-1) * g) / self.D_oracle           # (B,)
...
def run_schur_fullkernel(
    ...
    *,
    H_mode: str = "enumerate",        # "enumerate" or "probes"
    ...
    m_T[0] = 0.5  # initial teacher coefficient
    m_H = torch.zeros(len(H_list), device=device, dtype=torch.float32)

    # Self-consistent sparsity: initialize rho used inside α(w) (Acoef) to 0.5 (uninformative)
    rho_for_Acoef = 0.5
    ...
    for it in range(1, outer_steps+1):
        ...
        if H_mode == "probes" and (it == 1 or resample_probes_each_iter):
            ...
            all_modes = T_list + H_list
            all_idx_pad, all_mask, _ = modes_to_padded_idx(all_modes)
            C_all_i8 = chi_for_modes(X, all_idx_pad, all_mask)              # redefine (P, M_all)
            T_idx = torch.arange(0, len(T_list), device=device, dtype=torch.long)
            H_idx = torch.arange(len(T_list), len(all_modes), device=device, dtype=torch.long)
        ...
        # Pass a float32 view of C_all for SGLD (avoid re-casting inside)
        C_all_f = C_all_i8.float()

        # ---- E-step with sparsity-aware α(w): use rho_for_Acoef from previous outer step ----
        w, phi, D_oracle, D_internal, p_active = sgld_sample_w(
            w, X,
            C_all=C_all_f,
            y_vec=sS,
            m_full=m_full,
            ...
            mdl=mdl,
            mcmc=mcmc,
            rho_for_Acoef=rho_for_Acoef,    # α(w) uses N_eff = ρN
            prior_logit=0.0                 # uninformative gate prior
        )

        # Self-consistent gate level ρ for the NEXT outer step (mean activation)
        rho_cur = float(p_active.mean().item())
        rho_for_Acoef = rho_cur  # feed back into next α(w)

        # ---- Full-kernel oracle and Schur solve (M-step), gate-weighted ----
        oracle = FullKernelOracle(C_all_i8, sS, N=mdl.N)
        oracle.update_samples(phi, D_oracle, gate=p_active)  # gate-weighted contributions
        b_full = oracle.vec_b()
        ...
        # Closed-form single-coordinate diagnostics (teacher)
        with torch.no_grad():
        ...
        traj["diagB"].append(oracle.diag_B_scalar); traj["rho"].append(rho_cur)
    ...
    fname = f"schur_fullkernel_{tag}_kap{float(kappa):.3e}_P{P}_M{len(T_list)+len(H_list)}.json"
...
if __name__ == "__main__":
    ...
    P_list     = [10,100,500,1000,3000,5000,10000,20000]       # e.g. [100, 500, 1000, 2000]
    kappa_list = [1e-3]        # e.g. [1e-3, 1e-2]
    outer_steps = 2000         # <-- outer iterations (decoupled from SGLD steps)
    save_dir = "/home/goring/mean_field_langevin/MCMC_finiteP/results5/c4_1024_probe_sparse"
    run_tag  = ""

    # H split (lighter by default):
    H_card_small     = 4       # keep exact modes up to order-4
    H_per_card_high  = 0       # not used in probe mode

    # PROBE MODE SETTINGS
    H_mode = "probes"             # "enumerate" or "probes"
    R_probes_high = 1024
    ...
    # NOTE: ridge can remain tiny; gating already damps nuisance modes
    sol  = SolveParams(ridge_lambda=1e-9, pcg_tol_outer=1e-5, pcg_max_outer=400,
                       pcg_tol_inner=5e-4, pcg_max_inner=200, print_every=10,
                       saem_a0=1.0, saem_t0=150.0, saem_damping=3.0)

...
def nn_equiv_ridge_lambda(
    ...
    gamma: float = 1.0,
    *,
    solver_has_unit_identity: bool = True,
    clamp_nonneg: bool = True,
):
    """
    Compute the ridge that matches the Langevin NN.

    Uses:  λ_total = T / σ_w^2,  with  T = 2 κ^2 / P,  σ_w^2 = g_w / d.
    Here `kappa` is the (already scaled) κ used in MF (your code passes this directly).

    If your Schur system uses (1 + lam)•I + NB (as in your code), set
    solver_has_unit_identity=True so we return lam_for_solver = λ_total - 1.
    """
    # NN-equivalent total identity coefficient
    lambda_total = (2.0 * (kappa ** 2) / float(P)) * (d / float(g_w))

    if solver_has_unit_identity:
        lam = lambda_total - 1.0
        if clamp_nonneg:
            lam = max(lam, 0.0)
        return lam, lambda_total
    else:
        return lambda_total, lambda_total
    ...
    *,
    ...
    for kap in kappa_list:
        for P in P_list:
            ...
            lam_for_solver, lambda_total = nn_equiv_ridge_lambda(
                P=P,
                d=d,
                N=mdl.N,
                kappa=kap,              # you pass κ directly in this script
                g_w=mdl.sigma_w,        # in your MF code, sigma_w plays the role of g_w
                gamma=mdl.gamma,
                solver_has_unit_identity=True,  # your Schur uses (1 + lam)·I + NB
                clamp_nonneg=True
            )
            sol_this = replace(sol, ridge_lambda=float(lam_for_solver))
            print(f"[auto ridge] P={P} κ={kap:.3e} ⇒ λ_total={lambda_total:.3e}; "
                f"solver uses (1+lam) so lam={sol_this.ridge_lambda:.3e}")
            ...
            res, path = run_schur_fullkernel(
                ...
                mdl=mdl, mcmc=mcmc, sol=sol_this,              # <-- pass the per-run sol
...
if __name__ == "__main__":
    ...
    save_dir = "/home/goring/mean_field_langevin/MCMC_finiteP/results5/c4_50_enumerate"
    run_tag  = ""

    # H split (lighter by default):
    H_card_small     = 4       # keep exact modes up to order-2
    H_per_card_high  = 50       # not used in probe mode

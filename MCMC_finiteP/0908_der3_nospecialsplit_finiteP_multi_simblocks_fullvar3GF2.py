# 0908_der3_nospecialsplit_finiteP_multi_simblocks_matfreecg_FIXED.py
...
def compute_logp_and_grad_w_blocks_indep(
    ...
    Sigma = (phi.detach() * phi.detach()).mean(dim=1)

    # back-reaction
    J_beta = J[:, :, 0] - backreact_lambda * torch.einsum('kbm,km->kb', J, m_blocks)

    # fixed-D, no grad
    D = (A * (kappa**2) + Sigma).clamp_min(saem.eps_D).detach()

    prior_term = -0.5 * (w_blocks * w_blocks).sum(dim=2) / gw2
    data_quad  = 0.5 * (J_beta * J_beta) / ((kappa**2) * D)
...
# ----------------------------- Plain CG on a provided operator A -----------------------------

def cg_solve_operator(apply_A, rhs, tol=1e-6, max_iter=200):
    """
    Solve A x = rhs with CG, where apply_A(v) returns A v.
    Shapes: rhs (K, M).
    """
    device = rhs.device
    dtype  = rhs.dtype
...
def saem_multimode_quenched_fixedD_multi_runs(
    ...
    # Debug toggles
    DEBUG_DIAG_CHECK  = bool(_get(BASE, "debug_diag_check", False))
    DEBUG_MS_CHECK    = bool(_get(BASE, "debug_ms_check", False))
    ...
    for it in range(1, saem.max_iters + 1):
        ...
        for slot in per_dev:
            ...
            # B matvec (matrix-free); keep exactly one 1/B scaling
            ...
            if DEBUG_DIAG_CHECK:
                with torch.no_grad():
                    ...
                    col0 = matvec_B(v_e0)
                    print("diag(B)_mean vs (B e0)_S mean:",
                          float(diag_B[:, 0].mean().item()),
                          float(col0[:, 0].mean().item()))

            if DEBUG_SCALE_CHECK:
                with torch.no_grad():
                    v_e0 = torch.zeros(Kb, diag_B.shape[1], device=device, dtype=J1.dtype); v_e0[:, 0] = 1.0
                    lhs = apply_A(v_e0) - (1.0 + lam) * v_e0   # should be N * (B e0)
                    Be0 = matvec_B(v_e0)
                    ratio = (lhs[:, 0].abs().mean() / (Nf * Be0[:, 0].abs().mean() + 1e-30)).item()
                    print(f"[scale-check] mean |A(eS)-(1+lam)eS| / (N*B eS) ~ {ratio:.3f}  (want ~1.0)")

            # Solve A m = rhs
            m_star = cg_solve_operator(
                apply_A, rhs,
                tol=BASE.get("cg_tol", 1e-5),
                max_iter=BASE.get("cg_max_iter", 400)   # bump a bit for big M
            )
...
if __name__ == "__main__":
    ...
    BASE = dict(
        ...
        # debug toggles
        debug_diag_check=False,
        debug_ms_check=True,       # <--- turn on to see the 1D vs CG number
        debug_scale_check=True,    # <--- turn on to verify N is in the operator
        force_diag_in_cg=False,    # <--- set True to test diagonal-only coupling
    )

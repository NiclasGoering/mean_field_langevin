# rs_cavity_explicit_aw_ard_fast_reaction_fixed_ngamma_with_fp_metrics_lr_decay_FAST.py
# Full-batch SGLD, bf16 autocast, torch.compile, Anderson acceleration, cheap eval/logging.
# Reaction (1/N) and MALA removed.
...
class RSCavityExplicitMulti:
    ...
    def _make_eval_sets(self):
        """Pre-generate and cache routine (small) eval X; defer full eval X until needed."""
        d = self.mdl.d
    ...
    def _sgld_inner(self, X: torch.Tensor, r: torch.Tensor, eta: float):
        gw, ga, _ = self._stats_and_grads_stream(X, r, self.W, self.a, return_field=False)
...
if __name__ == "__main__":
    ...
    algo = Algo(
        ...
        step_size=1e-2,
        log_every=50000,            # less frequent logs
        batch_eval=131_072,
        P_chunk_train=4096*16,      # big chunks for better GPU utilization
        ...
        save_every_logs=3,     # write json every 10th log
    )
    alpha0 = 0.5
    beta01 = alpha0 / d
    ...
    out_dir = "/home/goring/mean_field_langevin/MCMC_composite/results_d35k4hm_paper_final_conv2_long_fin_ard0.5/5e-4"

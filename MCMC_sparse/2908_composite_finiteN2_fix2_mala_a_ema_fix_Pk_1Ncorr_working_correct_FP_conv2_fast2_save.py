...
class RSCavityExplicitMulti:
    def __init__(self, mdl: Model, algo: Algo, kappa: float, device: torch.device,
        ...
        # NEW: keep param seeds for snapshot metadata
        self.param_seeds = list(seeds_params)
        # NEW: record snapshot file paths
    ...
    # ---------------- NEW: Snapshot helpers for p(W,a) ----------------

    @torch.no_grad()
    def _compute_empirical_stats(self, W: torch.Tensor, a: torch.Tensor) -> Dict[str, Any]:
        """
        Compute per-experiment summary stats over particles (dimension 1).
        Inputs expected on CPU, float dtype.
          W: (E,B,d)
          a: (E,B,1)
        Returns small float32 tensors.
        """
        # Ensure float (we'll cast to float32 on outputs)
        W = W.float()
        a = a.float()

        E, B, d = W.shape

        # Means over particles
        w_mean = W.mean(dim=1)                 # (E,d)
        a_mean = a.mean(dim=1)                 # (E,1)

        # Centering
        Wc = W - w_mean[:, None, :]            # (E,B,d)
        ac = a - a_mean[:, None, :]            # (E,B,1)

        # Cov(W) per experiment: (E,d,d)
        w_cov = torch.zeros(E, d, d, dtype=torch.float32)
        for e in range(E):
            # (B,d) -> (d,d), population covariance (divide by B)
            w_cov[e] = (Wc[e].T @ Wc[e]) / float(B)

        # Var(a) per experiment: since a is scalar per particle, just the variance across B
        a_var = (ac.squeeze(-1) ** 2).mean(dim=1, keepdim=True).to(torch.float32)  # (E,1)

        # Cross-moment E[W_j * a] per experiment:
        # broadcast a from (E,B,1) to (E,B,d), then average over B → (E,d)
        wa_cross = (W * a.squeeze(-1)[:, :, None]).mean(dim=1).to(torch.float32)   # (E,d)

        # Per-particle L2 then average (useful quick checks)
        w_l2_mean = torch.linalg.vector_norm(W, dim=2).mean(dim=1, keepdim=True).to(torch.float32)  # (E,1)
        a_l2_mean = torch.linalg.vector_norm(a, dim=2).mean(dim=1, keepdim=True).to(torch.float32)  # (E,1)

        return {
            "w_mean": w_mean.to(torch.float32),
            "w_cov": w_cov,                 # already float32
            "a_mean": a_mean.to(torch.float32),
            "a_var": a_var,                 # float32
            "wa_cross": wa_cross,           # float32 (E,d)
            "w_l2_mean": w_l2_mean,         # float32
            "a_l2_mean": a_l2_mean,         # float32
        }
    ...
    def _save_particles(self,
                        ...
                        stage: str,
                        stride: int = 1,
                        to_dtype: Optional[torch.dtype] = torch.float32,
                        save_npz: bool = True) -> Dict[str, str]:
        """
        Save an empirical sample of p(W,a) (raw particles + summary stats).
        - base_prefix: filename stem (we'll append __{stage}_particles.*)
        - stage: "init" or "final"
        - stride: subsample particles along B (e.g., 2 saves every other particle)
        - to_dtype: cast saved particles to this dtype (None keeps native)
        - save_npz: also save a compressed NumPy mirror alongside the .pt
        """
        os.makedirs(os.path.dirname(base_prefix), exist_ok=True)

        # Subsample & CPU move
        W_cpu = self.W[:, ::stride, :].detach().to("cpu")
        a_cpu = self.a[:, ::stride, :].detach().to("cpu")
        rho_cpu = self.rho.detach().to("cpu")

        if to_dtype is not None:
            W_cpu = W_cpu.to(to_dtype)
            a_cpu = a_cpu.to(to_dtype)

        # Stats in float32
        stats = self._compute_empirical_stats(W_cpu.float(), a_cpu.float())
        ...
        meta = {
            ...
            "time_unix": time.time(),
            "E": int(self.E),
            "B_saved": int(W_cpu.shape[1]),
            "B_full": int(self.mdl.B),
            "d": int(self.mdl.d),
            "dtype_saved": str(W_cpu.dtype).replace("torch.", ""),
            "scale_f": float(self.scale_f),
            "kappa": float(self.kappa),
            "T": float(self.T),
            "device": str(self.device),
            "model": dict(self.mdl.__dict__),
            "algo": dict(self.algo.__dict__),
            "ard": {
                "use_ard": bool(self.ard.use_ard),
                "alpha0": float(self.ard.alpha0),
                "beta0": float(self.beta0),
                "ema": float(self.ard.ema),
                "rho_min": float(self.ard.rho_min),
                "rho_max": float(self.ard.rho_max),
            },
            "param_seeds": self.param_seeds,
        }

        pkg = {
            "params": {"W": W_cpu, "a": a_cpu, "rho": rho_cpu},
            "stats": stats,
            "meta": meta,
        }

        pt_path = f"{base_prefix}__{stage}_particles.pt"
        torch.save(pkg, pt_path)
        ...
        if save_npz:
            npz = {
                "W": W_cpu.numpy(),
                "a": a_cpu.numpy(),
                "rho": rho_cpu.numpy().astype(np.float32),
                "w_mean": stats["w_mean"].numpy(),
                "w_cov": stats["w_cov"].numpy(),
                "a_mean": stats["a_mean"].numpy(),
                "a_var": stats["a_var"].numpy(),
                "wa_cross": stats["wa_cross"].numpy(),
                "w_l2_mean": stats["w_l2_mean"].numpy(),
                "a_l2_mean": stats["a_l2_mean"].numpy(),
            }
            npz_path = f"{base_prefix}__{stage}_particles.npz"
            np.savez_compressed(npz_path, **npz)

        return {"pt": pt_path, "npz": npz_path}
    ...
    def run(self, X: torch.Tensor, y: torch.Tensor, out_dir: str, tag: str="", dev_tag: str=""):
        ...
        # NEW: base prefix for snapshot files (without extension)
        base_prefix = os.path.splitext(save_path)[0]
        # NEW: snapshot initial empirical p(W,a)
        self._snapshots["init"] = self._save_particles(
            base_prefix, stage="init", stride=1, to_dtype=torch.float32, save_npz=True
        )
        ...
        for it in range(1, self.algo.outer_steps+1):
            ...
            if (
                ...
                if test_mse_val < self.algo.early_stop_test_mse_threshold:
                    ...
                    # NEW: snapshot final before returning
                    self._snapshots["final"] = self._save_particles(
                        base_prefix, stage="final", stride=1, to_dtype=torch.float32, save_npz=True
                    )
                    ...
                    return {"path": save_path, "traj": hist, "snapshots": self._snapshots}
            ...
            if self.algo.early_stop_enabled:
                ...
                if best_counter >= self.algo.early_stop_patience:
                    ...
                    # NEW: snapshot final at composite early stop
                    self._snapshots["final"] = self._save_particles(
                        base_prefix, stage="final", stride=1, to_dtype=torch.float32, save_npz=True
                    )
                    payload["snapshots"] = self._snapshots
        ...
        # NEW: snapshot final at normal end
        self._snapshots["final"] = self._save_particles(
            base_prefix, stage="final", stride=1, to_dtype=torch.float32, save_npz=True
        )
        payload["snapshots"] = self._snapshots

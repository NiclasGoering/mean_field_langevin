# rs_cavity_intouta_MAP_stable.py
# RS self-consistent cavity with a integrated out, per-particle MAP on w only,
# throttled ARD on w (optional), EMA-damped μ, optional μ ridge + cap,
# and blended field updates. No 1/N reaction terms.
...
def poly_decay_lr(step: int, eta_start: float, eta_final: float, decay_steps: int, power: float = 2.0) -> float:
    """Polynomial decay from eta_start to eta_final over 'decay_steps' steps (clamped)."""
    if decay_steps <= 0:
        return eta_final
    tau = min(1.0, step / float(decay_steps))
...
class ARD:
    ...
    start_after: int = 2000                  # burn-in before ARD updates
    max_mul_per_update: float = 1.05         # multiplicative cap on rho growth per update

@dataclass
class Algo:
    outer_steps: int = 20000
    inner_mala_steps: int = 1               # number of GD micro-steps per outer step
    use_mala: bool = False                  # (unused; no MALA here)
    log_every: int = 100
    P_eval: int = 100_000
    batch_eval: int = 131072

    # chunking
    B_chunk: Optional[int] = None           # (unused; whole B fits)
    ...
    # ---- LR schedule for w ----
    eta_w_start: float = 1e-7
    eta_w_final: float = 1e-9
    lr_decay_steps: int = 50_000
    lr_power: float = 2.0

    # ---- NEW: stability knobs for integrated-out a ----
    mu_ema: float = 0.05      # EMA fraction for μ (0<mu_ema<=1). Like a step-size on a.
    mu_cap: Optional[float] = 25.0  # hard cap on |μ| (None to disable)
    mu_ridge: float = 0.0     # extra ridge on μ denominator (acts like larger σ_a)
    cg_like_update: bool = True
    field_blend: float = 0.1  # blend for f updates (smaller = more damping)

# ----------------------------- core ------------------------------

class RSCavityIntOutA:
    def __init__(self, mdl: Model, algo: Algo, kappa: float, device: torch.device,
        ...
        # parameters: W only (B,d) per experiment
        W_list = []
        for e in range(E):
        ...
        # ARD prior precision per-dim (E,d). Start at d/σ_w^2 (matches N(0, σ_w^2/d I) per coord).
        ...
        self.N_2gamma = self.mdl.N ** (2.0 * self.mdl.gamma)
        ...
        # μ memory for EMA
        self._mu_latest = torch.zeros(self.E, self.mdl.B, 1, device=self.device, dtype=self.dtype)

        # last-step Σ stats for logging
        self._last_sigma_mean = 0.0

    @staticmethod
    def _batched_mm_X_Wt(X: torch.Tensor, W: torch.Tensor) -> torch.Tensor:
        # X: (E,n,d), W: (E,B,d) -> (E,n,B)
    ...
    # -------------- gradients with a integrated out --------------

    @torch.no_grad()
    def _grads_intouta(self, X: torch.Tensor, r: torch.Tensor, W: torch.Tensor):
        """
        Compute per-exp, per-particle gradients on W with a integrated out.

        Definitions (on current residual r = y - ⟨f⟩, treated as constant):
          Σ_b      = E[ φ_b^2 ]
          J_r,b    = E[ φ_b r ]
          G1_b     = E[ φ_b φ'_b x ]                 (vector in ℝ^d)
          G2_b     = E[ φ'_b x r ]                   (vector in ℝ^d)
          den_b    = 1/σ_a^2 + Σ_b / (κ^2 N^{2γ})
          μ_b      = ( J_r,b / (κ^2 N^γ) ) / den_b            (with EMA/clip/ridge)
          var_b    = 1 / den_b

        Gradient:
          ∂U/∂w_b  = ( (var_b + μ_b^2)/(κ^2 N^{2γ}) ) * G1_b  -  ( μ_b/(κ^2 N^γ) ) * G2_b  +  ρ ⊙ w_b
        where ρ acts per-coordinate (ARD prior), applied elementwise to w_b.
        """
        E, P, d = X.shape
        B = W.shape[1]
        eps = torch.tensor(1e-30, dtype=self.dtype, device=self.device)

        Sigma_sum = torch.zeros(E, B, device=self.device, dtype=self.dtype)
        Jr_sum    = torch.zeros(E, B, device=self.device, dtype=self.dtype)
        G1_sum    = torch.zeros(E, B, d, device=self.device, dtype=self.dtype)
        G2_sum    = torch.zeros(E, B, d, device=self.device, dtype=self.dtype)
        ...
        for start in range(0, P, step):
            ...
            Sigma_sum += (Phi * Phi).sum(dim=1)   # (E,B)
            Jr_sum    += torch.bmm(Phi.transpose(1,2), rc).squeeze(-1)   # (E,B)

            # G1_b = E[ φ φ' x ]: (E,B,d)
            G1_sum += torch.bmm((Phi * dPhi).transpose(1,2), Xc)

            # G2_b = E[ φ' x r ]: (E,B,d)
            # r has shape (E, n, 1), dPhi is (E, n, B)
# (dPhi * r) -> (E, n, B), transpose to (E, B, n), then bmm with Xc (E, n, d) -> (E, B, d)
            G2_sum += torch.bmm((dPhi * rc).transpose(1, 2), Xc)


        invP = 1.0 / float(P)
        Sigma = Sigma_sum * invP                      # (E,B)
        Jr    = Jr_sum * invP                         # (E,B)
        G1    = G1_sum * invP                         # (E,B,d)
        G2    = G2_sum * invP                         # (E,B,d)

        # guards
        Sigma = torch.nan_to_num(Sigma, nan=0.0, posinf=0.0, neginf=0.0)
        Jr    = torch.nan_to_num(Jr,    nan=0.0, posinf=0.0, neginf=0.0)

        # μ and var from integrated-out posterior, with ridge + EMA + cap
        den = (1.0 / (self.mdl.sigma_a ** 2)) + (Sigma / ( (self.kappa ** 2) * self.N_2gamma ))
        if self.algo.mu_ridge and self.algo.mu_ridge > 0:
            den = den + float(self.algo.mu_ridge)
        den = torch.clamp(den, min=eps)

        num = Jr / ( (self.kappa ** 2) * self.N_gamma )

        mu_raw = (num / den).unsqueeze(-1)          # (E,B,1)
        var    = (1.0 / den).unsqueeze(-1)          # (E,B,1)

        # EMA damping of μ
        beta = float(self.algo.mu_ema)
        mu_prev = self._mu_latest
        mu = (1.0 - beta) * mu_prev + beta * mu_raw

        # Optional hard cap on |μ|
        if self.algo.mu_cap is not None:
            mu = torch.clamp(mu, min=-float(self.algo.mu_cap), max=float(self.algo.mu_cap))

        # stash for next iter
        self._mu_latest = mu

        # data gradient term
        coef1 = ( (var + mu*mu) / ( (self.kappa ** 2) * self.N_2gamma ) )  # (E,B,1)
        coef2 = ( mu / ( (self.kappa ** 2) * self.N_gamma ) )               # (E,B,1)

        grad_w = (coef1 * G1) - (coef2 * G2)                                # (E,B,d)

        # prior/ARD gradient
        grad_w = grad_w + (W * self.rho.unsqueeze(1))                       # (E,B,d)

        # clip if requested
        if self.algo.grad_clip_norm is not None:
            gw2 = (grad_w * grad_w).sum(dim=2, keepdim=True)
            gn = torch.sqrt(gw2) + 1e-12
            scale = torch.clamp(self.algo.grad_clip_norm / gn, max=1.0)
            grad_w = grad_w * scale

        # logging
        self._last_sigma_mean = float(Sigma.mean().item())

        return grad_w.to(self.dtype), mu.to(self.dtype)

    @torch.no_grad()
    def _field_from_mu_stream(self, X: torch.Tensor, W: torch.Tensor, mu: torch.Tensor) -> torch.Tensor:
        E, P, _ = X.shape
        ...
        for start in range(0, P, step):
            ...
            Z  = self._batched_mm_X_Wt(Xc, W)      # (E,n,B)
            Phi = activation(Z, self.mdl.act)      # (E,n,B)
            fa = torch.bmm(Phi, mu)                # (E,n,1)
            f[:, start:start+n] = (self.scale_f * fa.squeeze(-1))
        return f.unsqueeze(-1)  # (E,P,1)

    # ---------------- ARD (throttled) ----------------

    @torch.no_grad()
    def _update_rho_ard(self, it: int):
        if not self.ard.use_ard or (it < self.ard.start_after):
            return
        N = self.mdl.N
        alpha_post = self.ard.alpha0 + 0.5 * N
        ...
        rho_hat = alpha_post / torch.clamp(beta_post, min=eps)

        # multiplicative damping
        mul = torch.clamp(rho_hat / torch.clamp(self.rho, min=eps),
                          min=1.0/self.ard.max_mul_per_update,
                          max=self.ard.max_mul_per_update)
        rho_new = self.rho * mul

        # EMA + hard clamps
        self.rho.mul_(1.0 - self.ard.ema).add_(rho_new, alpha=self.ard.ema)
        self.rho.clamp_(self.ard.rho_min, self.ard.rho_max)
    ...
    def _eval_heldout(self, d: int, P_eval: int, chunk: int) -> Dict[str, Any]:
        ...
        for start in range(0, P_eval, step):
            ...
            Phi = activation(Z, self.mdl.act)                 # (E,n,B)
            f = (scale * torch.bmm(Phi, mu).squeeze(-1))      # (E,n)
    ...
    def run(self, X: torch.Tensor, y: torch.Tensor, out_dir: str, tag: str="", dev_tag: str=""):
        ...
        hist = {
            ...
            "eta_w": [], "eta_a": [],  # eta_a not used, keep for compatibility
            "c2_mean": [], "c2_eff_mean": [],  # c2_eff == c2 (no reaction)
        }
        t0 = time.time()

        ts = time.strftime("%Y%m%d_%H%M%S")
        fname = (
            f"rs_intouta_MAP_{tag or ts}_Ptr{P}_E{self.E}_Peval{self.algo.P_eval}_"
        ...
        for it in range(1, self.algo.outer_steps+1):
            # learning rate
            eta_w = poly_decay_lr(it, self.algo.eta_w_start, self.algo.eta_w_final,
                                  self.algo.lr_decay_steps, self.algo.lr_power)
            ...
            for _ in range(self.algo.inner_mala_steps):
                gw, mu = self._grads_intouta(X, r, self.W)
                self.W.add_(gw, alpha=-float(eta_w))
                self._clamp_params()

                if self.algo.kill_nan_particles:
                    bad = ~torch.isfinite(self.W).all(dim=2)
                    if bad.any():
                        bad_reset_sum += self._reinit_particles(bad)
                        if self.algo.log_bad_counts:
                            bad_prop_sum += int(bad.sum().item())

            # ARD update (on w)
            if self.ard.use_ard and (it % self.ard.update_every == 0):
                self._update_rho_ard(it)

            # update mean field using latest μ (with blending)
            f_new = self._field_from_mu_stream(X, self.W, self._mu_latest)
            ...
            if self.algo.early_stop_enabled:
                ...
                if best_counter >= self.algo.early_stop_patience:
                    ...
                    hist["c2_mean"].append(self._last_sigma_mean)
                    hist["c2_eff_mean"].append(self._last_sigma_mean)
                    hist["eta_w"].append(float(eta_w))
                    hist["eta_a"].append(0.0)
                    ...
                    payload = {
                        ...
                        "config": {
                            ...
                            "ard": {
                                ...
                                "start_after": self.ard.start_after,
                                "max_mul_per_update": self.ard.max_mul_per_update,
                            }
                        }
                    }
                    ...
                    print(json.dumps({
                        "iter": it, "mean_train_mse": train_mse_mean, "accept": acc_val,
...
def worker_process(dev_id: int, shard: List[Dict[str, Any]], mdl_dict: Dict[str, Any], algo_dict: Dict[str, Any], ard_dict: Dict[str, Any],
    ...
    for econf in shard:
        ...
        solver = RSCavityIntOutA(mdl, algo, kappa, device, E=E, seeds_params=param_seeds, teacher_sets=sets, ard=ard)
        tag = f"P{P}_kap{kappa:.3e}"
        dev_tag = f"dev{dev_id}"
        print(f"\n===== RUN start (MAP, a-integrated, no-reaction): P={P}, kappa={kappa:.6g}, E={E}, device={device}, dtype={'float64' if algo.use_float64 else 'float32'} =====")
...
if __name__ == "__main__":
    ...
    # Teacher: 4-sparse parity on first 4 dims
    teacher_sets_spec = "{0,1,2,3}"
    d = 35

    # Grids
    P_train_list = [1000,500,750]
    kappa_list   = [5e-3]
    num_exp = 3
    base_seed = 123456
    ...
    algo = Algo(
        ...
        P_chunk_train=4096*2,

        # LR on w
        eta_w_start=5e-7,  eta_w_final=1e-9,
        lr_decay_steps=25_000,
        lr_power=2.0,

        # Stability knobs for μ and f updates
        mu_ema=0.05,        # try 0.05–0.2; increase later for speed
        mu_cap=25000.0,        # relax/remove after stable
        mu_ridge=0.0,       # try 5.0 for more damping in early burn-in
        cg_like_update=True,
        field_blend=0.01,    # smaller = more damping of f
    )

    # Throttled ARD on w (optional)
    alpha0 = 0.01
    beta01 = alpha0 / d
    ard = ARD(
        use_ard=True,
        ...
        beta0=beta01,
        start_after=1000,
        max_mul_per_update=1.05,
    )

    out_dir = "/home/goring/mean_field_langevin/MCMC_composite/results_d35k4hm_test/test"

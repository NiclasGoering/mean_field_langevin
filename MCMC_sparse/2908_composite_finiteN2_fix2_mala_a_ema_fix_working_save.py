...
# PLUS: checkpointing of particles (W, a, rho) and a JSON that records those files.
...
class Algo:
    ...
    # NEW: checkpointing
    checkpoint_every: Optional[int] = None  # e.g., 1000 => save particles every 1000 outer steps
...
class RSCavityExplicit:
    ...
    @torch.no_grad()
    def save_checkpoint(self, path: str, with_rng: bool = False, extra: Optional[Dict]=None):
        """Save particles and config to a .pt file. Tensors are moved to CPU for portability."""
        payload = {
            "W": self.W.detach().cpu(),
            "a": self.a.detach().cpu(),
            "rho": self.rho.detach().cpu(),
            "kappa": self.kappa,
            "mdl": self.mdl.__dict__,
    ...
    def load_checkpoint(path: str, device: torch.device = torch.device("cpu")):
        """Load a checkpoint; returns (solver, ckpt_dict)."""
        ckpt = torch.load(path, map_location=device)
        mdl = Model(**ckpt["mdl"])
        algo = Algo(**ckpt["algo"])
        ard = ARD(**ckpt["ard"])
        solver = RSCavityExplicit(mdl, algo, ckpt["kappa"], device, teacher_sets=None, ard=ard)
        solver.W = ckpt["W"].to(device)
        solver.a = ckpt["a"].to(device)
        solver.rho = ckpt["rho"].to(device)
        return solver, ckpt
    ...
    def run(self, X: torch.Tensor, y: torch.Tensor, out_dir: str, tag: str=""):
        ...
        # NEW: record checkpoints we save along the way
        saved_ckpts: List[Dict[str, object]] = []
        ...
        for it in range(1, self.algo.outer_steps+1):
            ...
            # periodic logging & (NEW) checkpointing
            ...
            # NEW: save particle snapshots during training if requested
            if self.algo.checkpoint_every and (it % self.algo.checkpoint_every == 0):
                ck_name = f"particles_{tag or 'run'}_iter{it:07d}.pt"
                ck_path = os.path.join(out_dir, ck_name)
                self.save_checkpoint(ck_path, with_rng=False, extra={"iter": it, "elapsed_s": round(time.time()-t0,2)})
                saved_ckpts.append({"iter": it, "path": ck_path})
                print(f"[checkpoint] saved {ck_path}")

        # save final summary JSON + final checkpoint
        ...
        out = {
            ...
            # NEW: enumerate checkpoints saved during training
            "checkpoints": saved_ckpts,
        }

        # save final checkpoint and include it in JSON
        final_ck_name = f"particles_{tag}_final.pt"
        final_ck_path = os.path.join(out_dir, final_ck_name)
        self.save_checkpoint(final_ck_path, with_rng=False, extra={"iter": self.algo.outer_steps, "final": True})
        out["final_checkpoint"] = {"iter": self.algo.outer_steps, "path": final_ck_path}

        # write JSON
        json_path = os.path.join(
            out_dir,
            f"rs_cavity_explicit_aw_ard_{tag}_Ptr{X.shape[0]}_Peval{self.algo.P_eval}_kap{self.kappa:.3e}_N{self.mdl.N}_B{self.mdl.B}_g{self.mdl.gamma}.json"
        )
        with open(json_path, "w") as f:
            json.dump(out, f, indent=2)
        print(f"[saved] {json_path}")
        print(f"[saved] {final_ck_path}")
        return out
...
if __name__ == "__main__":
    ...
    algo = Algo(
        outer_steps=25000, inner_mala_steps=20, step_size=1e-7, use_mala=True,
        ...
        # ---- NEW: save particles regularly (helps with tail estimation) ----
        checkpoint_every=500,
    )
    kappa = 1e-3
    
    # ARD config (weak prior; EMA for stability)
    ard = ARD(use_ard=True, alpha0=1, ema=0.25, update_every=1,
              rho_min=0.0, rho_max=1e18, beta0=1/35)

    # NOTE: fix the path typo from your sample ("resutls" -> "results") if you want
    out_dir = "/home/goring/mean_field_langevin/MCMC_sparse/results/plot3"
    os.makedirs(out_dir, exist_ok=True)

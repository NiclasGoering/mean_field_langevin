# rs_cavity_explicit_aw_ard_fast_reaction_fixed_ngamma_MAP_opt_throttled_ard.py
# RS self-consistent cavity with explicit (a,w), per-particle MAP optimization,
# throttled ARD (burn-in + multiplicative cap + EMA), separate LR schedules for (w,a),
# and optional finite-N (1/N) Onsager/back-reaction corrections.
...
class ARD:
    ...
    # NEW: throttle + delay
    start_after: int = 2000                 # burn-in iters (outer steps) before ARD updates
    max_mul_per_update: float = 1.05        # multiplicative cap on rho growth per update

@dataclass
class Algo:
    outer_steps: int = 2000
    inner_mala_steps: int = 1              # reused as "inner GD steps" for MAP
    use_mala: bool = False                 # MAP optimization => False
    log_every: int = 10
    ...
    # ---- LR schedules (separate for w and a) ----
    eta_w_start: float = 2e-5
    eta_w_final: float = 2e-6
    eta_a_start: float = 5e-6
    eta_a_final: float = 5e-7
...
class RSCavityExplicitMulti:
    ...
    def _stats_and_grads_stream(self, X: torch.Tensor, r: torch.Tensor,
        ...
        # energy per particle (for MALA; still useful diagnostics)
    ...
    def run(self, X: torch.Tensor, y: torch.Tensor, out_dir: str, tag: str="", dev_tag: str=""):
        ...
        fname = (
            f"rs_aw_ard_multi_react_MAP_{tag or ts}_Ptr{P}_E{self.E}_Peval{self.algo.P_eval}_"
        ...
        for it in range(1, self.algo.outer_steps+1):
            ...
            for _ in range(self.algo.inner_mala_steps):
                self._gd_inner(X, r, eta_w, eta_a)
                if self.algo.log_bad_counts:
                    bad_prop_sum += 0
                    bad_reset_sum += 0

            # throttled ARD update
...
def worker_process(dev_id: int, shard: List[Dict[str, Any]], mdl_dict: Dict[str, Any], algo_dict: Dict[str, Any], ard_dict: Dict[str, Any],
    ...
    for econf in shard:
        ...
        print(f"\n===== RUN start (MAP): P={P}, kappa={kappa:.6g}, E={E}, device={device}, dtype={'float64' if algo.use_float64 else 'float32'} =====")
...
if __name__ == "__main__":
    ...
    # Grids
    P_train_list = [10000, 10, 100, 500, 750, 1000, 2133, 3666, 5000, 7500]
    kappa_list   = [7.5e-3, 1e-2, 1e-3, 5e-3, 7.5e-2, 2.5e-2, 5e-2, 5e-4] 
    num_exp = 3
    base_seed = 123456
    ...
    algo = Algo(
        outer_steps=75_000,
        inner_mala_steps=20,         # number of GD micro-steps per outer step
        use_mala=False,              # MAP optimization
        log_every=100,
        ...
        max_l2_w=None, max_l2_a=None,   # gentle cap for 'a' can help
        ...
        # --- 1/N reaction (stabilizes self-interaction) ---
        reaction_use=False,
        reaction_mode="lowrank_gram",           # or "exact_diag" if B is small
        reaction_update_every=10,
        ...
        # --- LR schedules ---
        eta_w_start=1e-7,  eta_w_final=1e-9,
        eta_a_start=1e-7,  eta_a_final=1e-9,
        lr_decay_steps=50_000,
        lr_power=2.0,
    )

    # Throttled ARD (burn-in + cap + smaller EMA + less frequent updates)
    alpha0 = 0.01
    beta01 = alpha0 / d
    ard = ARD(
        use_ard=True,
        alpha0=alpha0,
        ema=0.01,                 # smaller
        update_every=1,          # less frequent
        rho_min=0.0,
        rho_max=1e6,
        beta0=beta01,
        start_after=2500,         # burn-in
        max_mul_per_update=1.05,  # cap growth per update
    )

    out_dir = "/home/goring/mean_field_langevin/MCMC_composite/results_d35k4hm_new/alltest"

# rs_cavity_explicit_aw_ard_fast_reaction_fixed_ngamma_with_fp_metrics.py
...
#
# NEW (2025-09-05):
# - Optional linear scaling of step size with kappa (eta = step_size * kappa) via Algo.linear_kappa_step flag.
# - If training set size P <= 100, use 1/100 of the (possibly kappa-scaled) step size.
# - Added fixed-point residual metrics between f_new and f_mean, and ARD (rho) change metrics.
# - Early stopping now requires BOTH: (i) train MSE <= threshold AND (ii) fp_residual <= early_stop_fp_epsilon
#   for 'early_stop_patience' consecutive logged iterations.
...
class Algo:
    ...
    early_stop_fp_epsilon: float = 0.01  # NEW: tolerance for ||f_new - f_mean||
...
class RSCavityExplicitMulti:
    ...
    def _sgld_inner(self, X: torch.Tensor, r: torch.Tensor, eta: float):
        ...
        self.W.add_(torch.randn_like(self.W, dtype=self.dtype), alpha=math.sqrt(2.0*eta))   ##noise
    ...
    def run(self, X: torch.Tensor, y: torch.Tensor, out_dir: str, tag: str="", dev_tag: str=""):
        ...
        # ---------- NEW: compute effective step size once per run ----------
        eta = self.algo.step_size * (self.kappa**2 if self.algo.linear_kappa_step else 1.0)
        if P <= 100:
            # As requested: for small datasets (P <= 100), use 1/100 of the actual step size.
            eta *= 0.01
        ...
        hist = {
            ...
            # NEW: fixed-point & ARD metrics
            "fp_residual": [],  # scalar mean over experiments
            "fp_residual_per_exp": [],
            "rho_delta_rel": [],  # relative L2 change in rho
        }
        t0 = time.time()
        ...
        for it in range(1, self.algo.outer_steps+1):
            ...
            diff = (f_new - f_mean)
            fp_residual_per_e = torch.sqrt(torch.mean(diff*diff, dim=(1,2)))  # (E,)
            ...
            if self.ard.use_ard and (it % self.ard.update_every == 0):
                # compute change AFTER update vs previous
            ...
            # ----- now update f_mean (Picard/CG-like) -----
            ...
            # -------- early stopping (NEW joint criterion) --------
            if self.algo.early_stop_enabled:
                mse_ok = (train_mse_mean <= self.algo.early_stop_threshold)
                fp_ok = (fp_residual_mean <= self.algo.early_stop_fp_epsilon)
                if mse_ok and fp_ok:
                    best_counter += 1
                else:
                    best_counter = 0
                if best_counter >= self.algo.early_stop_patience:
                    ...
                    print(f"[early-stop] it={it} mean_train_mse={train_mse_mean:.6f} fp_residual={fp_residual_mean:.6e} — stopping early.")
                    print(json.dumps({
                        "iter": it, "mean_train_mse": train_mse_mean, "accept": acc_val if self.algo.use_mala else 0.0,
                        "fp_residual": fp_residual_mean, "rho_delta_rel": rho_delta_rel,
...
if __name__ == "__main__":
    ...
    kappa_list   =  [5e-4] #[7.5e-3, 1e-2, 1e-3, 5e-3, 7.5e-2, 2.5e-2, 5e-2, 5e-4]  7.5e-3,5e-3
    num_exp = 3
    base_seed = 123456
    ...
    algo = Algo(
        outer_steps=25_000, inner_mala_steps=50, step_size=1e-7, use_mala=False, #0005
        log_every=500, cg_like_update=True, field_blend=0.2,
        ...
        early_stop_fp_epsilon=0.01,   # NEW: joint criterion with MSE
        ...
        # --- NEW ---
        linear_kappa_step=False,  # set to False to disable eta ∝ kappa
    )
    alpha0 =25
    beta01 = alpha0 / d
    ...
    out_dir = "/home/goring/mean_field_langevin/MCMC_composite/results_d35k4hm_paper_final5/ard_25_inner50_5e-4"

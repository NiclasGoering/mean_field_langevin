# rs_cavity_explicit_aw_ard_fast.py
...
#
# Speed-focused version (keeps your hyperparams unchanged):
# - Streaming over P during training/eval to avoid giant (E,P,B) tensors.
# - No duplicated passes: grads + energy are computed together (current & proposal).
# - Replaces einsums with batched GEMMs (bmm) where beneficial.
# - Optional torch.compile hooks for the heavy streaming kernels.
# - Keeps logging cadence, P_eval, inner steps, step size, and use_mala exactly as you set.
...
class Algo:
    ...
    P_chunk_train: Optional[int] = 4096  # NEW: chunk over P during training
...
class RSCavityExplicitMulti:
    ...
    def _stats_and_grads_stream(self, X: torch.Tensor, r: torch.Tensor,
        ...
        term3_a = (C2 * (1.0 / (self.kappa**2 * float(P) * (self.N_gamma**2)))) * a_flat
        grad_a  = (term1_a + term2_a + term3_a).unsqueeze(2)  # (E,B,1)

        # w-grad (already scaled with -1/(kappa^2 P))
        ...
        # Energies per particle (E,B) using the same reductions:
        prior_w = 0.5 * (self.rho.unsqueeze(1) * (W * W)).sum(dim=2)       # (E,B)
        prior_a = 0.5 * (1.0 / (self.mdl.sigma_a**2)) * (a_flat ** 2)      # (E,B)
        data = ( - (a_flat / self.N_gamma) * C1 + 0.5 * (a_flat*a_flat / (self.N_gamma**2)) * C2 ) * (1.0 / (self.kappa**2 * float(P)))
    ...
    def _eval_heldout(self, d: int, P_eval: int, chunk: int) -> Dict[str, Any]:
        """Evaluation with streaming over P, unchanged numerics."""
        E = self.E
        device = self.device
        M = len(self.sets)
    ...
    def run(self, X: torch.Tensor, y: torch.Tensor, out_dir: str, tag: str="", dev_tag: str=""):
        ...
        fname = (
            f"rs_aw_ard_multi_{tag or ts}_Ptr{P}_E{self.E}_Peval{self.algo.P_eval}_"
        ...
        for it in range(1, self.algo.outer_steps+1):
            ...
            if self.algo.early_stop_enabled:
                ...
                if best_counter >= self.algo.early_stop_patience:
                    # Final eval + save snapshot before stopping
...
if __name__ == "__main__":
    ...
    algo = Algo(
        outer_steps=25_000, inner_mala_steps=200, step_size=2e-7, use_mala=False,
        ...
        P_chunk_train=4096*8,         # NEW: streaming chunk over P (does not change numerics)
    )
    alpha0=1e-6
    beta01=alpha0/35
    ard = ARD(use_ard=True, alpha0=alpha0, ema=0.01, update_every=1,
              rho_min=1e-12, rho_max=1e12, beta0=beta01)

    out_dir = "/home/goring/mean_field_langevin/MCMC_sparse/results/d35_k4_3108_a1grid_fix_nomala"  # change if needed

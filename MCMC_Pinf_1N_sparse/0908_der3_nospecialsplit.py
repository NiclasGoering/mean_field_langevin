...
def saem_solve_for_kappa(
    ...
    for it in range(1, saem.max_iters + 1):
        ...
        # Projections for feasibility (D>0) and bounds
...
if __name__ == "__main__":
    ...
    kappa_list = np.logspace(np.log10(1e-4), np.log10(5e1), 30)  #sorted(np.logspace(np.log10(1e-5), np.log10(5e1), 25), reverse=True)

    # Choose a save directory you have write access to
    save_dir = "/home/goring/mean_field_langevin/MCMC_Pinf_1N_sparse/results/1808_d40k4_nospecialst"
    os.makedirs(save_dir, exist_ok=True)

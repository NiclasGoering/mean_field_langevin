# quenched_multimode_feature_learning.py
...
# ----------------------------- J, Σ on a dataset block -----------------------------

def compute_J_Sigma_multimode(
    w: torch.Tensor, X: torch.Tensor, chi_mat: torch.Tensor, act: str
) -> Tuple[torch.Tensor, torch.Tensor]:
    """
    J_mat: (B, M) with J_A = (1/P) sum_p phi(x_p) chi_A(x_p)
    Sigma: (B,)   with Sigma = (1/P) sum_p phi(x_p)^2
    """
    z = X @ w.T                # (P,B)
...
# ----------------------------- logπ and grad (quenched datasets; with P) -----------------------------
...
def compute_logp_and_grad_multimode(
    ...
    """
    Quenched dataset of size P. Integrate out 'a' exactly (Gaussian):
      Acoef = N^γ / σ_a  (from prior 0.5*Acoef*a^2)
      C     = P / κ^2
      D     = Acoef + C * Σ
      B     = J_S - sum_A m_A J_A
      logp(w | dataset) = -||w||^2/(2 g_w^2) - 0.5*log D + 0.5 * C^2 * (B^2 / D)
    """
    ...
    J_S = J_mat[:, 0]                                                 # (B,)
    Bv = J_S - (J_mat @ m_vec)                                        # (B,)
    Dv = Acoef + C * Sigma                                            # (B,)
    D_safe = torch.clamp(Dv, min=saem.eps_D)

    prior_term   = -0.5 * (w * w).sum(dim=1) / gw2
    log_det_term = -0.5 * torch.log(D_safe)
    data_term    = 0.5 * (C * C) * (Bv * Bv) / D_safe
...
def mcmc_sgld_multimode(
    ...
    for _ in range(mcmc.n_steps):
        _, grad, J_mat, Sigma = compute_logp_and_grad_multimode(
            w, X, chi_mat, m_vec, kappa, P, mdl, saem, mcmc
        )
...
def saem_quenched_over_datasets_multimode(
    kappa: float, P: int, blocks_per_device: int, devices: List[int],
    ...
    mcmc = MCMCParams(
        ...
        n_steps=BASE.get("mcmc_steps", 400),
        ...
        clamp_w=BASE.get("clamp_w", 100.0),
    )
    saem = SAEMParams(
        ...
        print_every=BASE.get("print_every", 10),
    )
    ...
    # Build quenched dataset blocks on each device
    per_dev = []
    total_blocks = 0
    for di in devices if len(devices) > 0 else [-1]:
        ...
        blocks = []
        for b in range(blocks_per_device):
            seed_b = BASE.get("data_seed", 0) + 9973 * (b + 1) + 131 * (di + 1)
            Xb = make_boolean_batch(P, mdl.d, device, seed=seed_b)           # (P,d)
            chi_b = chi_of_modes(Xb, modes).to(device)                        # (P,Mtot)
            # init w ~ N(0, g_w^2 I)
            gw2 = mdl.sigma_w / mdl.d
            chains = mcmc.n_chains_per_device
            wb = torch.randn(chains, mdl.d, device=device) * math.sqrt(gw2)   # (B,d)
            blocks.append({"X": Xb, "chi_mat": chi_b, "w": wb})
        per_dev.append({"device": device, "blocks": blocks})
        total_blocks += blocks_per_device
    ...
    for it in range(1, saem.max_iters + 1):
        ...
        # E-step: cycle through blocks (datasets), SGLD per block, collect stats
        for slot in per_dev:
            device = slot["device"]
            for blk in slot["blocks"]:
                X, chi_mat, w = blk["X"], blk["chi_mat"], blk["w"]
                w, J_mat, Sigma = mcmc_sgld_multimode(
                    w, X, chi_mat, m_vec.to(device), kappa, P, mdl, saem, mcmc
                )
                blk["w"] = w  # persist chain state (quenched block)

                # D on device (with P)
                D = (Acoef + C * Sigma).clamp_min(saem.eps_D)     # (B,)
                invD = (1.0 / D).to(torch.float64)               # (B,)
                ...
                n_chain_total += J64.shape[0]

        # pooled averages over (chains × blocks)
        A_hat = (A_sum / max(1, n_chain_total)).to(torch.float32)  # (M,M)
        b_hat = (b_sum / max(1, n_chain_total)).to(torch.float32)  # (M,)

        # Robbins–Monro step size
        a_t = saem.a0 / (it + saem.t0)

        # Vector fixed-point residual for quenched datasets:
        # m = N * C * E[(J_S J)/D] - N * C * E[(J J^T)/D] m
        # => (I + N C A_hat) m = N C b_hat
        I = torch.eye(Mtot, dtype=torch.float32)
        F = (I + mdl.N * C * A_hat) @ m_vec - mdl.N * C * b_hat
        ...
        if it % saem.print_every == 0 or it == 1:
            # Diagnostic fixed point: (I + N C A_hat) m* = N C b_hat
            try:
                m_fixed = torch.linalg.solve(I + mdl.N * C * A_hat, mdl.N * C * b_hat)
            except RuntimeError:
                m_fixed = torch.nan * torch.ones_like(m_vec)

            dt = time.time() - t_start
            msg = {
                "iter": it, "kappa": kappa, "P": P, "blocks": total_blocks,
                "m_S": float(m_vec[0].item()),
                "m_S_fixed": float(m_fixed[0].item()) if torch.isfinite(m_fixed[0]) else None,
                "gen_err": float(0.5 * (1.0 - m_vec[0].item())**2),
    ...
    tag = run_tag if run_tag else f"kappa_{kappa:.6f}_P_{P}_B{total_blocks}"
    out_path = os.path.join(SAVE_DIR, f"saem_quenched_multimode_{tag}.json")
...
if __name__ == "__main__":
    ...
    save_dir = "./results_quenched_multimode"
    os.makedirs(save_dir, exist_ok=True)
    run_tag_prefix = time.strftime("%Y%m%d_%H%M%S")

    # Example sweeps
    kappa_list = [1e-3]      # noise level(s)
    P_list     = [100]  # dataset sizes
    blocks_per_device = 128               # increase for small P (e.g., 32–128)

    for kappa in kappa_list:
        # Optional warm-start across P
        m_ws = None
        for P in P_list:
            print(f"\n=== SAEM (quenched datasets, multi-mode) for kappa={kappa}, P={P}, blocks/dev={blocks_per_device} ===")
            if m_ws is not None:
                BASE["m_init"] = float(m_ws[0])  # warm-start teacher amplitude only
            tag = f"{run_tag_prefix}_kap{kappa:.3e}_P{P}_B{blocks_per_device}"
            result = saem_quenched_over_datasets_multimode(
                kappa=kappa, P=P, blocks_per_device=blocks_per_device,
                devices=devices, BASE=BASE, SAVE_DIR=save_dir, run_tag=tag
            )
            m_ws = result["m_final"]

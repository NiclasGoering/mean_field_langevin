...
def split_specialists(s: torch.Tensor, max_iter: int = 15) -> Tuple[torch.Tensor, torch.Tensor, float, Tuple[float,float]]:
    ...
    for _ in range(max_iter):
        ...
        # update
        if assign2.any():
            c2_new = s[assign2].mean()
        else:
            c2_new = c2
        if (~assign2).any():
            c1_new = s[~assign2].mean()
        else:
            c1_new = c1
    ...
    # threshold halfway between centers
...
def saem_solve_for_kappa(
    ...
    for it in range(1, saem.max_iters + 1):
        ...
        max_rho_batch = 0.0
        # also track s-centers/thresholds for logs
        taus = []; centers = []
        ...
        for slot in per_dev:
            ...
            # core statistics
            s = (J * J) / D                                 # ⇐ statistic for splitting
            g1 = (J * J) / D                                # per-chain g1 contribution
            g2 = ((J * J) * (J * J)) / ((kappa**2) * (D * D))

            # split into basins
            mask_s, mask_b, tau, (c_low, c_high) = split_specialists(s)
        ...
        # finalize g's per basin (avoid zero counts)
        if cnt_s > 0:
            g1_s = sum_g1_s / cnt_s
            g2_s = sum_g2_s / cnt_s
        else:
        ...
        # fraction observed (not used as the state, but for logging)
        p_emp = cnt_s / max(1, (cnt_s + cnt_b))

        # Mixture TAP residuals using current p
        ...
        # lever rule target for p (derived)
...
if __name__ == "__main__":
    ...
    # From large to small kappa for warm start
    kappa_list = np.logspace(np.log10(1e-5), np.log10(5e1), 40)

    save_dir = "/home/goring/mean_field_langevin/MCMC_Pinf_1N_sparse/results/0708_d30k4_diagosntic4_mix2"
    os.makedirs(save_dir, exist_ok=True)

    run_tag_prefix = time.strftime("%Y%m%d_%H%M%S")

    # Warm start across kappas
    m_ws, chi_ws, p_ws = BASE.get("m_init", 0.0), BASE.get("chi_init", 1e-6), BASE.get("p_init", 0.01)

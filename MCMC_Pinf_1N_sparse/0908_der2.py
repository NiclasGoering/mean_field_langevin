...
def compute_logp_and_grad(
    ...
    # Correct constant from integrating out a:
    # D(w) = (kappa^2 / sigma_a^2) + Sigma(w) - (chi/N) J(w)^2
    A = 1.0 / (mdl.sigma_a ** 2)
    ...
    # Correct Gaussian prior: -||w||^2/(2 sigma_w^2)
    prior_term = - (1.0 / (2.0 * (mdl.sigma_w ** 2))) * (w * w).sum(dim=1)

    # log-determinant and signal terms
...
def saem_solve_for_kappa(
    ...
    for di in devices if len(devices) > 0 else [-1]:
        ...
        # Initialize from the actual prior
        w = torch.randn(chains, mdl.d, device=device) * mdl.sigma_w
        per_dev.append({"device": device, "X": X, "chi_vec": chi_vec, "w": w})
        total_chains += chains

    # Init and PR averages
    m = BASE.get("m_init", 0.0)
    chi = BASE.get("chi_init", 1e-6)
    p = BASE.get("p_init", 0.01)
...
if __name__ == "__main__":
    ...
    # You can override with your favorite schedule
    kappa_list = np.logspace(np.log10(1e-5), np.log10(5e1), 20)

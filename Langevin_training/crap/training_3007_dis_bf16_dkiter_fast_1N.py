...
def train_with_langevin(model, X_train, y_train, X_test, y_test, hyperparams, current_config, device, use_full_batch):
    ...
    # --- IMPORTANT: use kappa as-is; T = 2*kappa^2 / P_train
    kappa = kappa_0
    T = 2.0 * (kappa ** 2) 
...
def worker(global_rank,
           ...
           gamma_values_sorted,   # ascending (or any order you pass)
           P_values_sorted_desc,  # descending
           shared_status,         # manager dict: status map
           skip_state,            # manager dict of manager dicts: per-gamma thresholds
    ...
    - Skip/prune frontier is tracked PER-GAMMA so different gammas don't interfere.
    """
    try:
    ...
    def g_idx_of(gamma: float) -> int:
        return gamma_values_sorted.index(gamma)

    def p_idx_of(P: int) -> int:
        return P_values_sorted_desc.index(P)

    def status_key(d, k, exp, k_idx, p_idx, g_idx) -> str:
        return f"{d}:{k}:{exp}:{k_idx}:{p_idx}:{g_idx}"

    # should we skip this job based on current skip thresholds? (PER-GAMMA)
    def should_skip_job(P, kappa_0, gamma_val):
        g_idx = g_idx_of(gamma_val)
        with shared_lock:
            active_map = skip_state['active']
            kthr_map = skip_state['kappa_idx_threshold']
            Pthr_map = skip_state['P_threshold']
            if not bool(active_map.get(g_idx, False)):
                return False
            k_idx = k_idx_of(kappa_0)
            kthr = int(kthr_map.get(g_idx, 1 << 30))
            Pthr = int(Pthr_map.get(g_idx, -1))
            return (k_idx >= kthr) and (P <= Pthr)

    # After a failure at (k_idx, p_idx), decide whether to expand skip region (PER-GAMMA).
    # Require neighbor (k_idx-1, p_idx-1) to ALSO be failure (if it exists).
    def maybe_expand_skip_region(d, k, exp_id, k_idx, p_idx, g_idx):
        if k_idx <= 0 or p_idx <= 0:
            return  # no neighbor exists; don't prune yet

        neighbor_key = status_key(d, k, exp_id, k_idx - 1, p_idx - 1, g_idx)
        with shared_lock:
            neighbor_status = shared_status.get(neighbor_key, None)
            if neighbor_status == "failure":
                cur_P = P_values_sorted_desc[p_idx]
                # read current thresholds for this gamma index
                active_map = skip_state['active']
                kthr_map = skip_state['kappa_idx_threshold']
                Pthr_map = skip_state['P_threshold']

                old_kthr = int(kthr_map.get(g_idx, 1 << 30))
                old_Pthr = int(Pthr_map.get(g_idx, -1))

                new_kthr = min(old_kthr, k_idx) if old_kthr != (1 << 30) else k_idx
                new_Pthr = max(old_Pthr, cur_P)

                active_map[g_idx] = True
                kthr_map[g_idx] = new_kthr
                Pthr_map[g_idx] = new_Pthr

                print(f"[SKIP REGION UPDATE | gamma_idx {g_idx}] Now skipping all with "
                      f"kappa_idx >= {new_kthr} and P <= {new_Pthr}.")
            else:
                pass
    ...
    while True:
        ...
        if should_skip_job(P_train, kappa_0, gamma_scaling_exponent):
            print(f"[GPU {device}] SKIP job (P={P_train}, k0={kappa_0:.3e}, gamma={gamma_scaling_exponent}) due to frontier rule. Writing random chance.")
        ...
        g_idx = g_idx_of(gamma_scaling_exponent)
        key = status_key(d, k, exp_id, k_idx, p_idx, g_idx)
        ...
        if not success:
            # require neighbor (k_idx-1, p_idx-1) also failure before pruning (per-gamma)
            maybe_expand_skip_region(d, k, exp_id, k_idx, p_idx, g_idx)
...
def main():
    ...
    # --- Save directory ---
    base_save_dir = Path("/home/goring/mean_field_langevin/Langevin_training/results_paper1/d35_k4_biggridtest")
    ...
    P_values = [5000]#[10, 100, 500,750, 1000,2133,3666, 5000, 7500, 10000, 20000]
    P_values = sorted(P_values, reverse=True)  # descending

    # kappa ascending (start from smallest kappa)
    kappa_0_values =  [0.05] #[7.5e-3,2.5e-2,7-5e-2, 1e-2, 1e-1, 1e-3, 5e-3,7.5e-2,2.5e-2, 5e-2, 5e-4]
    kappa_0_values = sorted(kappa_0_values)   # ascending

    # gamma values (ITERABLE)
    gamma_values = [0.5,1.0]  # add more, e.g., [0.0, 0.25, 0.5, 1.0]
    ...
    # --- Build job queue in the requested order (kappa asc, P desc, gamma any order you prefer) ---
    job_queue = mp.Queue()
    job_count = 0
    for d in d_values:
        for k in k_values:
    ...
    # key = "d:k:exp:k_idx:p_idx:g_idx"  -> "success" | "failure"
    shared_status = manager.dict()

    # Skip region thresholds PER-GAMMA
    # Using manager dicts keyed by gamma index so different gammas don't interfere.
    skip_state = manager.dict()
    skip_state['active'] = manager.dict()
    skip_state['kappa_idx_threshold'] = manager.dict()
    skip_state['P_threshold'] = manager.dict()
    for g_idx in range(len(gamma_values)):
        skip_state['active'][g_idx] = False
        skip_state['kappa_idx_threshold'][g_idx] = 1 << 30  # large
        skip_state['P_threshold'][g_idx] = -1               # small

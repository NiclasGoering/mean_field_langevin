...
def main():
    ...
    # --- Save directory ---
    base_save_dir = Path("/home/goring/mean_field_langevin/Langevin_training/results_paper1/d30_kall_lrdecay_5e-3_2")
    base_save_dir.mkdir(exist_ok=True, parents=True)

    # --- Experiment Grids (ORDER MATTERS) ---
    d_values = [30]
    k_values = [2,4,6,8,10,14,18,22,26,30]  # k <= d

    # P descending (start from largest P)
    P_values = [10, 100, 500, 750, 1000, 5000, 7500, 10000,25000,50000,75000,100000]  #
    P_values = sorted(P_values, reverse=True)  # descending

    # kappa ascending (start from smallest kappa)
    kappa_0_values = [5e-3]#[7.5e-3, 1e-2, 1e-3, 5e-3, 7.5e-2, 2.5e-2, 5e-2, 5e-4]  # more: [7.5e-3, 1e-2, 1e-1, 1e-3, 5e-3, 7.5e-2, 2.5e-2, 5e-2, 5e-4]

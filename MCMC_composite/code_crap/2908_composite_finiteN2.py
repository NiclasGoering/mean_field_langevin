...
def sgld_sample_w(
    ...
    D_or  = κ^2 N^{2γ} α = κ^2 N^{2γ} * (1/( σ_a^2)) + Σ_eff
    """
    device = w.device
    P = X.shape[0]
    N = float(mdl.N)
...
class FunctionalCavitySolver:
    ...
    def compute_reaction_lowrank(
        ...
        if self.react.use_corr:
            ...
            M2 = Rho.square() * (std_b.view(-1,1)**2) * (std_j.view(1,-1)**2)                                # (B,r)
        else:
    ...
    def run(self, X_train: torch.Tensor, y_train: torch.Tensor,
        ...
        for it in range(1, outer_steps + 1):
            ...
            if M_comp > 0:
                ...
                # λ_eff from reaction
                lam = float(rstats["lambda_eff"])

                # TAP update (your requested form): m <- (1-λ)m + λ*( drive + χ̂ ⊙ m )
                m_prev = self._m_S
...
if __name__ == "__main__":
    ...
    mcmc = MCMCParams(
        B=1024*4, steps=1200, step_size=2e-6, step_decay=1.0,
        grad_clip=0.0, clamp_w=0.0, autocast=False
    )
    sol  = SolveParams(
        outer_steps=3500, saem_a0=0.2, saem_t0=250.0, saem_damping=0.3, #80
        print_every=2
    )
    ...
    react = ReactionParams(
        ...
        anneal_iters=300,
        rho_clip=0.95,
        clamp_sigma_eff_min=1e-6 
    )

    P_TRAIN_LIST = [2500]#[100, 500, 1000, 2500, 5000, 7500, 10000, 20000]
    KAPPA_LIST   = [7.5e-3]  # start with the problematic small kappa

    results_dir = "/home/goring/mean_field_langevin/MCMC_composite/resutls_mf1/test_2908_1Ncorrection_test"

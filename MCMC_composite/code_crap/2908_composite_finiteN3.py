...
#
# NEW: Global flag to disable ALL 1/N stability/correction machinery and
#      only solve the m self-consistency equation (mean-field / no reaction).
#      Set MEAN_FIELD_ONLY = True below to enable this mode.
...
@dataclass
class GlobalFlags:
    # When True, disable ALL 1/N reaction/stability corrections (no low-rank reaction,
    # no ratio clip, no EMA, no annealing, no Σ_eff clamp). We still estimate statistics
    # with SGLD but treat Σ_eff ≡ Σ (i.e., R_divN=0). We then *only* iterate the
    # m self-consistency equation using lambda_react as its step size.
    mean_field_only: bool = False
...
def sgld_sample_w(
    ...
    clamp_sigma_eff_min: Optional[float] = 1e-3,
):
    """
...
class FunctionalCavitySolver:
    def __init__(self, mdl: ModelParams, mcmc: MCMCParams, sol: SolveParams,
                 ...
                 flags: Optional[GlobalFlags] = None):
        self.mdl = mdl
        self.mcmc = mcmc
        self.sol = sol
        self.kappa = kappa
        ...
        self.flags = flags or GlobalFlags()
        # lag-1 state
        self._R_divN_prev: Optional[torch.Tensor] = None
        self._J_indices: Optional[torch.Tensor] = None
        # TAP mode state (initialized in run when M is known)
        self._m_S: Optional[torch.Tensor] = None
        self._chi_fifo: Optional[deque] = None
        self._chi_avg_window: int = 3  # average χ over 2–4 minibatches (use 3)
    ...
    def compute_susceptibility_AA(
        ...
        """
        χ̂_AA (diagnostic) with the exact requested rescaling:
            χ̂_AA = (κ² / (N² γ)) * E_b[ (1/α_b) * J_{A,b}² ]
        where α_b = D_oracle_b / (κ² N^{2γ}),   J_{A,b} = E_x[C_A(x) φ_b(x)].
        No extra N factor.
        """
        if C_train.numel() == 0:
        ...
        for start in range(0, B, chunk_B):
            stop = min(start + chunk_B, B)
            g_chunk = phi_train[:, start:stop].to(torch.float32)              # (P, bchunk)
            J_chunk = (Ct @ g_chunk) / float(P)                               # (M, bchunk)
            D_chunk = D_oracle[start:stop].to(torch.float32)                  # (bchunk,)
            inv_alpha = (self.kappa ** 2 * N2g) / (D_chunk + 1e-30)           # (bchunk,)
            term = (inv_alpha.view(1, -1) * (J_chunk * J_chunk))              # (M, bchunk)
            accum += term.sum(dim=1)

        mean_term = accum / float(B)                                          # (M,)
        chi_AA = (self.kappa ** 2 / (N * N * max(gamma, 1e-12))) * mean_term  # (M,)
        return chi_AA
    ...
    def run(self, X_train: torch.Tensor, y_train: torch.Tensor,
        ...
        # TAP mode + χ̂ state
        if M_comp > 0:
            self._m_S = torch.zeros(M_comp, device=self.device, dtype=torch.float32)
            self._chi_fifo = deque(maxlen=self._chi_avg_window)

        traj = {
            "iter": [], "time_s": [],
            "train_mse": [], "train_corr": [], "train_y2": [], "train_f2": [],
            "eval_mS_vec": [], "eval_mS_norm2": [],
            "eval_coeff_mse": [], "eval_coeff_mae": [], "eval_coeff_sign_acc": [], "eval_noise_norm2": [], "eval_mse_total": [], "eval_corr_yf": [], "eval_R2": [],
            ...
            # TAP diagnostics (added)
            "tap_g_var": [], "tap_g_mode": [], "tap_sigma_eff_over_sigma": [], "tap_delta_m_norm": [], "tap_mS_vec": [], "tap_mS_norm2": [],
            # Flags
            "mean_field_only": []
        }
        t0 = time.time()
        ...
        for it in range(1, outer_steps + 1):
            # SGLD with either:
            #   - full lag-1 reaction & Σ_eff clamp (default), or
            #   - plain mean-field (R=0, no clamp) if mean_field_only=True
            R_for_sgld = (torch.zeros(B, device=self.device, dtype=torch.float32)
                          if self.flags.mean_field_only else self._R_divN_prev)
            clamp_for_sgld = (None if self.flags.mean_field_only else self.react.clamp_sigma_eff_min)

            W, phi_train, D_or_old, JY, Jm, Sigma_raw, Sigma_eff_lag = sgld_sample_w(
                W, X_train, y_train, f_mean, self.kappa, self.mdl, self.mcmc,
                R_divN=R_for_sgld, clamp_sigma_eff_min=clamp_for_sgld
            )
            Jr = (JY - Jm).to(torch.float32)

            # possibly resample probe set (ignored if mean_field_only)
            if not self.flags.mean_field_only:
            ...
            # Reaction update:
            if self.flags.mean_field_only:
                # Completely disable reaction: R_divN ← 0; λ_eff for TAP update uses lambda_react directly.
                R_divN_new = torch.zeros_like(Sigma_raw)
                rstats = {
                    "lambda_eff": float(self.react.lambda_react),
                    "mean_rsq": 0.0,
                    "mean_a2_react": 0.0,
                    "raw_mean_R_divN": 0.0,
                    "clipped_mean_R_divN": 0.0,
                }
            else:
                # Low-rank connected reaction with annealing/EMA/ratio-clip/clamp
            ...
            # corrected D_or & c (using new reaction), with optional Σ_eff clamp (disabled in mean-field mode)
            Sigma_eff_now = Sigma_raw - R_divN_new
            if (not self.flags.mean_field_only) and (self.react.clamp_sigma_eff_min is not None):
            ...
            # update lag-1 reaction (no-op in mean-field mode since we always pass zeros into SGLD)
            ...
            # ------------------ TAP mode update over composite characters ------------------
            if M_comp > 0:
                # J_A,b = E_x[C_A(x) φ_b(x)]
                Ct = C_train.to(torch.float32).t()                      # (M,P)
                J_full = (Ct @ phi_train.to(torch.float32)) / float(P_train)  # (M,B)

                # Drive term: E_b[c_b * J_{A,b}]
                drive = (J_full * c.view(1, -1)).mean(dim=1)            # (M,)

                # χ̂_AA estimate with exact rescaling and minibatch averaging
                N = float(self.mdl.N)
                gamma = float(self.mdl.gamma)
                N2g = N ** (2.0 * gamma)
                inv_alpha_b = (self.kappa ** 2 * N2g) / (D_or_corr + 1e-30)   # (B,)
                chi_est = (self.kappa ** 2 / (N * N * max(gamma, 1e-12))) * \
                          ((J_full * J_full) * inv_alpha_b.view(1, -1)).mean(dim=1)  # (M,)

                self._chi_fifo.append(chi_est)
                chi_hat = torch.stack(list(self._chi_fifo), dim=0).mean(dim=0)       # (M,)

                # TAP update:
                # if mean_field_only: use lambda_react directly as m-step size (no annealing)
                lam = float(self.react.lambda_react) if self.flags.mean_field_only else float(rstats["lambda_eff"])

                m_prev = self._m_S
                rhs = drive + chi_hat * m_prev
                m_new = (1.0 - lam) * m_prev + lam * rhs
                delta_m = m_new - m_prev
                self._m_S = m_new.detach()

                # TAP diagnostics
                tap_sigma_ratio = float((Sigma_eff_now / (Sigma_raw + 1e-12)).mean().item())
                tap_g_var = float(chi_hat.mean().item())
                tap_g_mode = float((drive.norm(p=2) / (m_prev.norm(p=2) + 1e-12)).item())
                tap_delta_norm = float(delta_m.norm(p=2).item())
                traj["tap_g_var"].append(tap_g_var)
                traj["tap_g_mode"].append(tap_g_mode)
                traj["tap_sigma_eff_over_sigma"].append(tap_sigma_ratio)
                traj["tap_delta_m_norm"].append(tap_delta_norm)
                traj["tap_mS_vec"].append(self._m_S.detach().cpu().tolist())
                traj["tap_mS_norm2"].append(float(self._m_S.pow(2).sum().item()))
            else:
                traj["tap_g_var"].append(float("nan"))
                traj["tap_g_mode"].append(float("nan"))
                traj["tap_sigma_eff_over_sigma"].append(float("nan"))
                traj["tap_delta_m_norm"].append(float("nan"))
                traj["tap_mS_vec"].append([])
                traj["tap_mS_norm2"].append(0.0)
            # ------------------ end TAP mode update ------------------
            ...
            # susceptibility on modes (optional parity diag; uses exact rescaling now)
            if M_comp > 0:
            ...
            traj["mean_field_only"].append(bool(self.flags.mean_field_only))
            ...
            if it % self.sol.print_every == 1 or it == outer_steps:
                ...
                print(json.dumps({
                    ...
                    # TAP printouts
                    "tap_g_var": traj["tap_g_var"][-1],
                    "tap_g_mode": traj["tap_g_mode"][-1],
                    "tap_sigma_eff_over_sigma": traj["tap_sigma_eff_over_sigma"][-1],
                    "tap_delta_m_norm": traj["tap_delta_m_norm"][-1],
                    "tap_mS_norm2": traj["tap_mS_norm2"][-1],
                    "mean_field_only": bool(self.flags.mean_field_only),
...
if __name__ == "__main__":
    ...
    # ---- Global toggle: mean-field only (disable ALL 1/N corrections) ----
    # Set to True to turn OFF reaction, ratio clip, EMA, annealing, and Σ_eff clamp.
    ...
    react = ReactionParams(
        r=4096,
        resample_every=5,
        a2_mode="full",          # connected susceptibility by default (kept; ignored if mean-field-only)
        use_corr=True,
        center_phi=True,
        lambda_react=0.02,      # also used as TAP step size in mean-field-only mode
        ema=0.99,
        anneal_iters=100,
        rho_clip=0.9,
    ...
    flags = GlobalFlags(mean_field_only=MEAN_FIELD_ONLY)
    ...
    for gamma in GAMMA_LIST:
        for P_train in P_TRAIN_LIST:
            ...
            for kappa in KAPPA_LIST:
                print(f"\n=== RUN: P_train={P_train}, kappa={kappa:.3e}, gamma={gamma}, mean_field_only={MEAN_FIELD_ONLY} ===")
                mdl  = ModelParams(d=d, N=N, k=0, sigma_a=1.0, sigma_w=1.0, gamma=gamma, act="relu")

                solver = FunctionalCavitySolver(
                    mdl, mcmc, sol, kappa=kappa, device=device,
                    teacher_sets=teacher_sets, react=react, flags=flags
                )

                gstr = f"{gamma:.6g}".replace('.', 'p').replace('-', 'm')
                run_tag = f"P{P_train}_kap{float(kappa):.3e}_g{gstr}" + ("_MFONLY" if MEAN_FIELD_ONLY else "")
                ...
                all_runs.append({
                    ...
                    "kappa": float(kappa),
                    "gamma": float(gamma),
                    "mean_field_only": bool(MEAN_FIELD_ONLY),
                    "path": os.path.join(
                        results_dir,

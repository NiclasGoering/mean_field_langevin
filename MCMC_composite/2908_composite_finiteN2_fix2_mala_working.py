...
class BasisFreeCavitySolver:
    ...
    def run(self, X: torch.Tensor, y: torch.Tensor,
        ...
        traj = {
            ...
            # MALA diagnostics (windowed accept rate at print)
            "mala_accept_rate": []
        }
        t0 = time.time()

        # counters for MALA acceptance over the last print window
        accept_window = 0
        ...
        for it in range(1, self.sol.outer_steps + 1):
            ...
            Sigma_eff = Sigma - (2*R_divN_prev if self.sol.use_reaction else 0.0)
            ...
            with torch.no_grad():
                ...
                eta = self.mcmc.step_size                  # fixed step size for MALA
            ...
            Sigma_eff_prop = Sigma_prop - (R_divN_prev if self.sol.use_reaction else 0.0)
            ...
            # ensure next autograd build starts clean
            ...
            with torch.no_grad():
                ...
                Sigma_eff_env = Sigma_env - (2*R_divN_prev if self.sol.use_reaction else 0.0)
...
if __name__ == "__main__":
    ...
    mcmc = MCMCParams(B=16384, step_size=9e-6, grad_clip=0.0, clamp_w=0.0, langevin_sqrt2=True)
    sol  = SolveParams(
        outer_steps=5000000, print_every=100, use_reaction=False,
        cg_tol=1e-7, cg_maxit=200, ridge=0.0,
        P_eval=50_000, eval_chunk=8192*32
    )

    solver = BasisFreeCavitySolver(mdl, mcmc, sol, kappa=kappa, device=device, teacher_sets=sets)
    _ = solver.run(X, y, log_dir="/home/goring/mean_field_langevin/MCMC_composite/resutls_mf1/d35_k4_false_5000",

...
class Model:
    ...
    sigma_a: float = 1.0
    sigma_w: float = 1.0
    sigma_b: float = 1.0   # NEW: std for hidden bias prior
    act: str = "relu"
...
class RSCavityExplicitMulti:
    def __init__(self, mdl: Model, algo: Algo, kappa: float, device: torch.device,
        ...
        for e in range(E):
            ...
            be = torch.randn(mdl.B, 1, generator=g, device=device, dtype=self.dtype) * mdl.sigma_b  # NEW: bias per particle
        ...
        self.b = torch.stack(b_list, dim=0)  # (E,B,1)  NEW
        ...
        self._buf_Hb = torch.empty(self.E, self.mdl.B, device=self.device, dtype=self.dtype)  # NEW: bias grad accumulator
    ...
    def _ensure_full_eval_set(self):
        ...
        for e in range(self.E):
            ...
            if self.teacher_ws is not None:
                ...
                ze = Xe @ we
                gz = _hermite_he(ze, self.hermite_degree)
                #Ye = torch.where(gz >= 0, torch.tensor(1.0, device=self.device, dtype=self.dtype),torch.tensor(-1.0, device=self.device, dtype=self.dtype)).unsqueeze(1)
                Ye = gz.unsqueeze(1)
                Ys.append(Ye)
    ...
    def _reinit_particles(self, mask: torch.Tensor):
        ...
        sb = self.mdl.sigma_b * self.algo.nan_reinit_std_scale  # NEW
        total = 0
        for e in range(self.E):
            m = mask[e]
            if m.any():
                ...
                self.b[e, m] = torch.randn(n, 1, generator=g, device=self.device, dtype=self.dtype) * sb  # NEW
        return total
    ...
    def _stats_and_grads_stream(self, X: torch.Tensor, r: torch.Tensor,
        ...
        with ctx:
            for start in range(0, P, step):
                ...
                Z   = Z + self.b.transpose(1, 2)                   # (E,1,B) -> broadcast across n   NEW
                ...
                Hb += M.sum(dim=1).to(self.dtype) * (-2.0 * invP)                 # NEW: bias grad data term
    ...
    def _field_from_particles_stream(self, X: torch.Tensor) -> torch.Tensor:
        ...
        with ctx:
            for start in range(0, P, step):
                ...
                Z  = Z + self.b.transpose(1, 2)  # NEW: add bias
    ...
    def _sgld_inner(self, X: torch.Tensor, r: torch.Tensor, eta: float, return_field: bool = False):
        ...
        self.b.add_(torch.randn_like(self.b, dtype=self.dtype), alpha=nstd)  # NEW

        if self.algo.kill_nan_particles:
            bad_now = (~torch.isfinite(self.W).all(dim=2)) | (~torch.isfinite(self.a).all(dim=2)) | (~torch.isfinite(self.b).all(dim=2))  # NEW
            if bad_now.any():
...
if __name__ == "__main__":
    ...
    P_train_list =  [50,100,1000,5000,10000,25000,50000,75000]#[100,500,1000,5000]#[500, 1000, 10000, 2133, 10, 100, 750, 3666, 5000, 7500]
    ...
    use_float64 = False
    mdl = Model(d=d, B=1024, N=1024, gamma=0.5, sigma_a=1.0, sigma_w=0.5, sigma_b=1.0, act="relu")  # NEW: sigma_b configurable
    algo = Algo(
    ...
    out_dir = "/home/goring/mean_field_langevin/Index/results/2009_MF_d18_k2_p4_a=0.1_correct/1e-5"
